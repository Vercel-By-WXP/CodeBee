...
- **预估改为「开跑前点名基线 + 终态事后对账」，不设进队阻断**：同类型内部成本差 15~114 倍（实测 serial_novel 中位 12.1 万 / P90 185 万 tok），类型级预估切不开「正常长任务」与「跑飞」，所以开跑前不否决——阻断仍由 `budget.max_tokens_per_run` 在真实累计用量越线时判。改动三处：「全部」档不再把预估坍缩成 480 秒常数、快照优先取 P90 并落样本数（「近 90 天 · 1 次同类」与「按流程基线估算」在 UI 上分得开），run 进终态那一刻把预估与真实 token/耗时写成一行对账记录（含比值、是否超 P90、快照来源），攒一个观察窗口再评估要不要收紧。
