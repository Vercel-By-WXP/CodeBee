...
- 所有由按钮触发的耗时写操作统一显示局部旋转态与页面顶部进度，并在等待期间拦截重复点击；成功、失败和超时后都会恢复原按钮状态。
- 检查更新、导入供应商、继续连载、诊断导出等较慢只读操作接入同一反馈；后台轮询、控制权心跳、自动更新检查和自动重绑保持静默，不会误锁无关按钮。
- 新增真实浏览器回归测试，覆盖 POST/GET 即时反馈、防连点、异常恢复及后台请求静默。
