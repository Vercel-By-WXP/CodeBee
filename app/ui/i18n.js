...
(function () {
  ...
  const EN = {
    ...
    "群机器人 Webhook（钉钉 / 飞书 / 企业微信，按域名自动适配）": "Group bot webhook (DingTalk / Feishu / WeCom, auto-adapted by domain)",
    "https://oapi.dingtalk.com/robot/send?access_token=…": "https://oapi.dingtalk.com/robot/send?access_token=…",
    "任务收尾把结果摘要推到群里；也接受任意 https 地址（按钉钉 text 形状发，自建 n8n 等自选）。": "Pushes the run summary to your group chat on task finish; any https URL is accepted (sent in DingTalk text shape, for self-hosted n8n etc.).",
    "个人推送（推到你的手机，配任意一条即可）": "Personal push (to your phone; any one channel is enough)",
    "Bark Device Key": "Bark device key",
    "Bark 服务器（默认 https://api.day.app）": "Bark server (default https://api.day.app)",
    "ntfy 主题 URL（如 https://ntfy.sh/my-topic）": "ntfy topic URL (e.g. https://ntfy.sh/my-topic)",
    "Server酱 SendKey（推送到微信）": "ServerChan SendKey (pushes to WeChat)",
    "Telegram Bot Token": "Telegram bot token",
    "Telegram Chat ID": "Telegram chat ID",
    "密钥只存本机。多通道同时配置时全部发送；任一通道收到即视为送达。": "Keys stay on this machine. All configured channels receive every push; any one delivery counts.",
    "保存推送设置": "Save push settings",
    "发送测试": "Send test",
    "推送设置已保存": "Push settings saved",
    "发送中…": "Sending…",
    "部分通道失败：": "Some channels failed: ",
    "✓ 测试消息已发出（": "✓ Test message sent (",
    "全部通道发送失败：": "All channels failed: ",
    // —— 流程分享码（任务类型管理）——
    "导出": "Export",
    "导入分享码": "Import share code",
    "导出失败：": "Export failed: ",
    "分享码已复制到剪贴板": "Share code copied to clipboard",
    "把这段分享码发给任何人，对方在「任务类型管理 → 导入分享码」粘贴即可一键安装：": "Send this share code to anyone — they paste it under \"Task type manager → Import share code\" for one-click install:",
    "📤 导出流程：": "📤 Export flow: ",
    "粘贴他人分享的流程码。若与现有流程同 ID：预置流程写入可回滚的自定义改动，自定义流程将被覆盖。": "Paste a share code from someone else. Same-ID conflicts: built-in flows get a rollback-able override, custom flows are overwritten.",
    "📥 导入流程分享码": "📥 Import flow share code",
    "请先粘贴分享码": "Paste a share code first",
    "（与现有内容一致）": " (identical to current content)",
    "（已覆盖现有定义）": " (existing definition overwritten)",
    "已导入流程「": "Imported flow \"",
    // —— 模型评测（评测基准台）——
    "模型评测": "Model Bench",
    "固定样题（章节写作 / 修 bug / 结构化摘要）让候选模型各跑一遍，同一评审链打分出能力榜；修 bug 题带客观验证（代码真跑断言）。评审用「编排者」供应商，先在编排设置里配好；裁判与候选同供应商时如实标注「同族评审」。所有调用计入用量台账（评测记账不计费用，价格表单位未核实）。实测分（14 天内）已作为软信号参与模型链排序与推荐选模——只动排序，健康/冷却/协议等硬约束照常一票否决。": "Fixed samples (chapter writing / bug fix / structured summary) run against every candidate model, then one judging chain scores them into a capability leaderboard; the bug-fix sample also gets objective verification (the code actually runs against assertions). Judging uses the \"Orchestrator\" provider — configure it in Orchestration Settings first. Same-provider judges are honestly tagged \"same-family review\". All calls go into the usage ledger (bench records tokens but no cost — price-table units unverified). Fresh bench scores (within 14 days) act as a soft signal in model-chain ranking and recommendations — ordering only; hard constraints like health/cooldown/protocol still veto as usual.",
    "内置样题": "Built-in samples",
    "候选模型（勾选要实测的模型）": "Candidates (tick the models to test)",
    "开始评测": "Start bench",
    "停止": "Stop",
    "能力榜": "Leaderboard",
    "带客观验证": "objective check",
    "裁判：": "Judge: ",
    "裁判未就绪：先在「编排设置」配置编排者供应商": "Judge not ready: configure the orchestrator provider in \"Orchestration Settings\" first",
    "评测中 ": "Running ",
    "同族评审": "same-family judge",
    "未得分": "unscored",
    "综合 ": "Overall ",
    "样题 ": "samples ",
    "　客观验证 ": "  objective pass ",
    ...
    "已注入 ": "injected ",
    "吸收 ": "absorbed ",
    " 个近似题": " near-dup titles",
    "被近似题合并吸收的变体标题数": "Variant titles absorbed by near-duplicate merging",
    "做法": "Best practice",
    ...
    "跳过法定节假日": "Skip statutory holidays",
    "法定休假日不认领新 Bug，扫描顺延到下一个工作日（对账照跑）；调休补班的周末照常扫描": "No new bugs are claimed on statutory holidays and the scan is deferred to the next workday (reconciliation still runs); make-up workday weekends are scanned as usual",
    "，法定休假日自动顺延": ", deferred on statutory holidays",
