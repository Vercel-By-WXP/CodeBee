...
(function () {
  ...
  const EN = {
    ...
    "自动提炼（草稿）": "auto-distilled (draft)",
    "——每次任务结束由编排者从产出材料里提炼知识条目，你确认「转正」后才参与注入；": "— after each run the orchestrator distills knowledge entries from output material; they join prompt injection only after you approve them;",
    "手动新建（直接转正）": "manually created (approved)",
    "——也是你自己的知识管理库。转正条目按任务类型与相关性自动注入同类任务的规划与评审提示词。": "— doubles as your personal knowledge base. Approved entries are injected by task type and relevance into planning/review prompts of similar tasks.",
    ...
    "还没有知识条目——完成一次产出型任务后自动提炼（草稿态待转正），也可以点右上角手动新建。": "No entries yet — finish an output-producing task and entries are auto-distilled (drafts awaiting approval), or create one manually.",
    "（暂无）": "(none)",
    "皮肤": "Skins",
    "手机连接": "Phone link",
    "设置": "Settings",
