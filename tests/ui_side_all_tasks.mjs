 ...
 * 树形 + 紧凑时间式样：任务行（字形/省略标题/时间）单行即按钮（无内嵌步骤层）。
 * 覆盖：4 个任务行全在（刷屏任务/窗口外任务/两个从未运行的任务）、活跃任务排前、
 *       侧栏无步骤层、点任务行主栏直开任务详情且行高亮。
...
async function main() {
  ...
  try {
    ...
    const og = JSON.parse(await evalJs(`JSON.stringify((() => {
      ...
      const glyphOf = (d) => { const g = d.querySelector(".sglyph");
      ...
      return {
        ...
        layerGone: { stepx: document.querySelectorAll("#side-tasks .stepx").length,
    ...
    check("侧栏无内嵌步骤层（.stepx/.smore 全为 0）",
      og.layerGone.stepx === 0 && og.layerGone.smore === 0, JSON.stringify(og.layerGone));
    check("窗口外任务（5 小时前）在侧栏：完成绿点", og.keys.includes("task-quiet") && og.quietGlyph === "ok",
      JSON.stringify({ keys: og.keys, quietGlyph: og.quietGlyph }));
    check("从未运行的两个任务在侧栏：暗点", og.norun.length === 2 && og.norun.every((x) => x.g === "none"),
      JSON.stringify(og.norun));

    /* ---- B) 点窗口外任务行 → 主栏直开它自己的任务详情，行高亮 ---- */
    await evalJs(`(() => {
      const rows = [...document.querySelectorAll("#side-tasks .stask")];
      rows.find((d) => d.dataset.key === "task-quiet").click();
      return "ok";
    })()`);
    await sleep(1200);   // 详情渲染；行高亮吃 2s 轮询重绘，这里强制重绘直验类逻辑
    ...
    check("点窗口外任务行：主栏直开它的任务详情且行高亮",
      opened.detailShown && opened.detailTaskKey === "task-quiet" &&
      opened.inspectorHidden && opened.activeKey === "task-quiet", JSON.stringify(opened));
    await evalJs(`(() => { S.sideSig = null; renderSideTasks(); return "ok"; })()`);
    await sleep(400);
    const hl = JSON.parse(await evalJs(`JSON.stringify((() => {
      const act = document.querySelector("#side-tasks .stask.active");
      return { title: document.getElementById("page-title").textContent,
        detailShown: !document.getElementById("run-detail").classList.contains("hidden"),
        activeKey: act ? act.dataset.key : "" };
    })())`));
    check("重绘后任务详情仍开且窗口外任务行保持高亮", hl.detailShown && hl.activeKey === "task-quiet", JSON.stringify(hl));
