...
async function main() {
  ...
  try {
    ...
    //    同步断言「点击立即有反馈」：两阶段安装的第一步是预览检查，
    //    mkrInstall 的同步段先翻按钮+toast 再发预览请求（SSRF 对本地地址秒拒，
    //    300ms 后已是完成态，中间态必须同步抓）。
    ...
    check("点击同步生效：按钮变检查中并禁用", fb.btn && fb.btn.includes("检查中") && fb.off === true, JSON.stringify(fb));
    check("点击同步生效：toast 提示下载检查中", /下载/.test(fb.toast), fb.toast);
    await sleep(1200);
