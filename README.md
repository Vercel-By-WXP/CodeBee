...
- README 全面重写：功能导览 + 实机截图，新增蜂巢/经验库/插件市场/自动化/对话模式等章节
- 修复部分机器上孤儿 CLI 清扫拖慢启动 60 秒的问题（改后台执行，不再挡就绪）
- 连载更稳：死链自动补位活链 CLI、限流退避窗口可视化、答疑单步轮不重跑
- 目录页管理四道防线；评审全挂不再误判未达标；新增欢迎引导与手工添加模型
<!-- relnotes:end -->
