...
- 智能体目录新增三款 CLI：Google **Gemini CLI**、腾讯云 **CodeBuddy CLI**（`cbc`）、字节开源 **Trae Agent**（`trae-cli`）——目录页一键安装/升级与无头编排，凭据走各 CLI 自带登录态
- 防假绑定：这三款暂不支持「CLI 绑定」链（走本机凭据），配置了供应商链会被明确拦截提示，不再出现「链显示活着、实际走 CLI 默认模型」
<!-- relnotes:end -->
...
- **17 种任务类型**：直接执行、代码、小说、连载小说、自媒体文章、短视频脚本、
  文档、翻译、扫榜选材、缺陷复盘、调研报告、演讲稿、工作汇报、商务邮件、技术方案、简历、标书编制；
  全部支持自定义流程（引擎 / 评审维度 / 阈值 / 轮数 / 提示词覆盖）；
...
阈值、轮数、产出文件名、起草/评审提示词覆盖）。内置 17 种；禅道工单是自动转成
...
| 📈 扫榜选材 | direct | 抓七猫/番茄/起点/纵横四平台榜单 → 跨平台题材/人设/切入点洞察 |
| 🕓 缺陷复盘 | direct | 读禅道/Jira 导出 → 产品/开发/测试三视角复盘报告 |
| 🔍 调研报告 | review | 起草 → 多维评审（全面性/深度/论据/结论）→ 修订 → 门禁 |
| 🎤 演讲稿 | review | 起草 → 多维评审（主题/结构/感染力/语言）→ 修订 → 门禁 |
| 📅 工作汇报 / ✉️ 商务邮件 / 📋 技术方案 / 📄 简历 | review | 各自专属角色、交付约束与评审维度 |
| 📑 标书编制 | review | 招标文件对齐评分点 → 多维评审（应答/合规/针对性）→ 修订 → 门禁 |
...
CLI 默认冒烟、供应商可达性、wire 适配、单模型生成、绑定路由与完整任务验收各有不同的
通过条件；统一错误分类、换路规则和近期失败复盘见 [执行与评测标准](docs/execution-standard.md)。
